...
# Reuse existing nodebuilder-suitecrm container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-suitecrm; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-suitecrm -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/mariadb:10.11.5 docker.io/bitnami/suitecrm:8.6.2" \
    "${container}"
# Commit the image
